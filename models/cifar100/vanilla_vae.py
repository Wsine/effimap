...
class VanillaVAE(nn.Module):
    ...
    def loss_function(self, x, query, x_reconstructed):
        mean, logvar = query
...
def get_model(ctx, pretrained):
    vae = VanillaVAE(
        label='', image_size=32, channel_num=3, kernel_num=128, z_size=128
    )
    if pretrained is True:
        state = load_torch_object(ctx, 'vae_model.pt')
...
class TestVanillaVAE(unittest.TestCase):
    def setUp(self) -> None:
        self.model = get_model(None, pretrained=False)
    ...
    def test_forward(self):
        ...
        y = self.model(x)
        print("Model Output size:", y[1].size())
    ...
    def test_loss(self):
        ...
        result = self.model(x)
        loss = self.model.loss_function(x, result)
        print(loss)
